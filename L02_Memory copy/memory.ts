...
let memory: string[] = ["A", "A", "B", "B", "C", "C", "D", "D", "F", "F"];

let memoryValues: [] = [];
let memoryTiles: [] = [];
let tilesFlipped: number = 0;

// tslint:disable-next-line: no-any

Array.prototype.memory_shuffle = function (): void {
    let j: number;
    let x: string;
    for (let i: number = this.length; i > 0; i--) {
...
function memoryFlipTile(tile, val): void {
            ...
            if (memoryValues.length == 0) {
                ...
                if (memoryValues[0] == memoryValues[1]) {
                    ...
                    if (memoryValues.length == 0) {
                        ...
                        // tslint:disable-next-line: typedef
                        } else {function flip2Back() {
                            // Flip the 2 tiles back over
                            let tile_1: any = document.getElementById(memoryTiles[0]);
                            // tslint:disable-next-line: no-any
                            var tile_2: any = document.getElementById(memoryTiles[1]);
